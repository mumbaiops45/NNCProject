...
//               Want <span style={{ textDecoration: "underline", textDecorationColor: "rgba(255,255,255,0.4)" }}>MOBILE APP DEVELOPMENT solutions</span> that take your business to the next level?
//             </h2>
...
//             {/* Q1: How much does a mobile app cost in Canada or UK? */}
...
//                   <span style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: 700, color: faq === 0 ? "#fff" : "rgba(255,255,255,0.78)", lineHeight: 1.4 }}>How much does a mobile app cost in Canada or UK?</span>
//                 </div>
...
//                   <span style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: 700, color: faq === 1 ? "#fff" : "rgba(255,255,255,0.78)", lineHeight: 1.4 }}>How long does mobile app development take?</span>
//                 </div>
...
//             {/* Q3: Do you publish to the App Store and Google Play? */}
...
//                   <span style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: 700, color: faq === 2 ? "#fff" : "rgba(255,255,255,0.78)", lineHeight: 1.4 }}>Do you publish to the App Store and Google Play?</span>
//                 </div>
...
//                   <span style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: 700, color: faq === 3 ? "#fff" : "rgba(255,255,255,0.78)", lineHeight: 1.4 }}>Can the app work offline?</span>
//                 </div>
...
//             <h2 style={{
//               fontSize: isMobile ? "clamp(24px,5vw,28px)" : isTablet ? "32px" : "clamp(36px,3.5vw,42px)",
//               fontWeight: 900,
//               color: "#fff",
//               letterSpacing: "-0.02em",
//               lineHeight: 1.2,
//               marginBottom: 12
//             }}>
//               Ready to Build <GradText>Next-Level Custom Digital Solutions?</GradText>
//             </h2>
...
const FAQS = [
  { q: "How much does a mobile app cost in Canada or the UK?", a: "A simple MVP mobile app (single platform, 5–8 screens) starts from CAD $12,000–$22,000 / £9,000–£17,000. A mid-complexity app with backend, push notifications, and integrations ranges from CAD $28,000–$65,000 / £22,000–£52,000. Enterprise apps with CRM integration, offline sync, and multi-platform support are scoped individually — typically CAD $70,000+ / £55,000+. All quotes are fixed-price, with no scope creep surprises." },
  { q: "How long does mobile app development take?", a: "A focused MVP on a single platform takes 8–12 weeks. A cross-platform app with backend and integrations typically runs 14–22 weeks. Enterprise apps with complex integrations, offline-first architecture, and multi-platform builds are scoped individually — usually 20–36 weeks. Every project starts with a fixed milestone plan delivered before work begins." },
  { q: "Do you publish to the App Store and Google Play?", a: "Yes — we handle full App Store (iOS) and Google Play (Android) submission and approval. This includes preparing app store listings, screenshots, preview videos, privacy policy integration, age rating compliance, and responding to reviewer feedback. We also advise on App Store Optimisation (ASO) to maximise organic downloads." },
  { q: "Can the app work offline?", a: "Yes. We build offline-first architecture when required — particularly for field service apps, healthcare apps, and logistics tools used in low-connectivity environments. Data is stored locally using SQLite or AsyncStorage and synced automatically to the backend when internet is restored. Conflict resolution logic ensures no data is lost during sync." },
  { q: "Do you build for both iOS and Android?", a: "Yes. We build native iOS (Swift/SwiftUI), native Android (Kotlin), and cross-platform apps (React Native, Flutter). For most businesses, we recommend React Native or Flutter as the most cost-effective option — giving you a single codebase, near-native performance, and simultaneous iOS and Android delivery." },
  { q: "Are your mobile apps GDPR, PIPEDA, and CCPA compliant?", a: "Yes. Compliance is built in from day one. For UK and EU clients we implement GDPR consent flows, in-app data deletion requests, encrypted storage, and audit-ready data handling. For Canadian clients we follow PIPEDA. For US clients we implement CCPA opt-out flows. Privacy policy generation, App Store privacy nutrition labels, and compliance documentation are included with every delivery." },
];
...
function buildParams(data: { firstName: string; lastName: string; email: string; phone: string; service: string; message: string }) {
  return {
    ...
    title: `New Mobile App Consultation from ${data.firstName} ${data.lastName}`.trim(),
    reply_to: data.email,
  };
}
...
export default function MobileAppDevelopmentPage() {
  ...
  const getSectionPadding = () => isMobile ? "40px 16px" : isTablet ? "60px 32px" : "40px 48px";
  const getHeroPadding = () => isMobile ? "40px 16px 60px" : isTablet ? "80px 32px 80px" : "90px 48px 80px";
  const getHeroFontSize = () => isMobile ? "28px" : isTablet ? "36px" : "48px";
    ...
    <>
      ...
      <style jsx global>{`
        ...
        *,*::before,*::after{box-sizing:border-box;}
        ...
        .btn-teal,.btn-outline,.btn-white{display:inline-flex;align-items:center;justify-content:center;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:700;font-family:'Poppins',sans-serif;cursor:pointer;text-decoration:none;transition:all .2s;min-width:140px;}
        @media(min-width:768px){.btn-teal,.btn-outline,.btn-white{padding:14px 32px;font-size:15px;min-width:160px;}}
        @media(max-width:640px){.btn-teal,.btn-outline,.btn-white{width:100%;min-width:0;}}
        ...
        .ss-tab.act{border-color:rgba(0,201,167,0.45);background:rgba(0,201,167,0.1);color:${T};}
        .ss-tab:hover:not(.act){border-color:rgba(255,255,255,0.2);color:rgba(255,255,255,0.8);}
        .svc-card{position:relative;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:18px;padding:24px 20px;display:flex;flex-direction:column;gap:12px;transition:transform .3s,border-color .3s,background .3s;cursor:default;overflow:hidden;}
        .svc-card:hover{transform:translateY(-6px);border-color:rgba(0,201,167,0.35);background:rgba(0,201,167,0.04);}
        .svc-card:hover .svc-icon{background:rgba(0,201,167,0.18)!important;transform:scale(1.08);}
        .pt-card{border-radius:18px;padding:20px 16px;position:relative;overflow:hidden;transition:transform .28s,box-shadow .28s;cursor:default;}
        .pt-card:hover{transform:translateY(-7px);box-shadow:0 20px 50px rgba(0,0,0,0.4);}
        .pt-card:hover .pt-icon{transform:scale(1.1) rotate(-5deg)!important;}
        .kb-card{position:relative;display:flex;gap:20px;align-items:flex-start;padding:24px 20px;border-radius:20px;border:1px solid rgba(255,255,255,0.07);background:rgba(255,255,255,0.02);transition:transform .3s,border-color .3s;cursor:default;animation:kbFadeUp .6s ease both;}
        .kb-card:hover{transform:translateY(-6px);border-color:rgba(0,201,167,0.3);}
        .ai-feat-card:hover{transform:translateX(6px)!important;}
        .wcu-point:hover{border-color:rgba(0,201,167,0.25)!important;background:rgba(0,201,167,0.04)!important;transform:translateX(6px)!important;}
        .wcu-stat:hover{transform:translateY(-4px)!important;background:rgba(0,201,167,0.1)!important;}
        .faq-item{border-radius:14px;border:1px solid rgba(255,255,255,0.07);background:rgba(255,255,255,0.02);overflow:hidden;transition:border-color .25s,background .25s;cursor:pointer;}
        .faq-item.fopen{border-color:rgba(0,201,167,0.35);background:rgba(0,201,167,0.04);}
        ...
        @media(max-width:960px){.svc-grid{grid-template-columns:repeat(2,1fr)!important;}.two-col-wide{grid-template-columns:1fr!important;}.wcu-stats{grid-template-columns:1fr 1fr!important;}}
        @media(max-width:768px){section{padding:60px 24px!important;}.two-col{grid-template-columns:1fr!important;gap:20px!important;}.kb-grid{grid-template-columns:1fr!important;}.cf-grid{grid-template-columns:1fr!important;gap:40px!important;}.cf-name{grid-template-columns:1fr!important;gap:12px!important;}}
        @media(max-width:720px){.pt-grid{grid-template-columns:repeat(2,1fr)!important;}}
        @media(max-width:640px){section{padding:40px 20px!important;}.stats-row{grid-template-columns:repeat(2,1fr)!important;}.hero-layout{padding:0!important;}}
          ...
          <div style={{ background: "rgba(8,20,40,0.85)", border: "1px solid rgba(0,201,167,0.22)", borderRadius: 20, padding: isMobile ? "24px 16px" : "32px 28px", backdropFilter: "blur(16px)", boxShadow: "0 32px 80px rgba(0,0,0,0.5),0 0 40px rgba(0,201,167,0.06)", position: "relative", overflow: "hidden" }}>
            ...
            {submitted ? (
              <div style={{ textAlign: "center", padding: isMobile ? "20px 0" : "40px 0" }}>
                ...
                <h3 style={{ color: "#fff", fontSize: isMobile ? 18 : 20, fontWeight: 800, marginBottom: 10 }}>Request Received!</h3>
                <p style={{ color: "rgba(255,255,255,0.5)", fontSize: isMobile ? 13 : 14, lineHeight: 1.7, marginBottom: 24 }}>We&apos;ll contact you within 24 hours with a free consultation.</p>
                <button onClick={() => setSubmitted(false)} className="btn-teal" style={{ minWidth: "auto", padding: isMobile ? "10px 22px" : "11px 26px", fontSize: isMobile ? "13px" : "14px" }}>Send Another →</button>
              </div>
            ) : (
              <>
                <div style={{ marginBottom: isMobile ? 14 : 22 }}>
                ...
                <form onSubmit={handleSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: isMobile ? 10 : 12 }}>
                  ...
                  <div>
                    ...
                    <input className={`fi${hTouched.email && hErr.email ? " fi-err" : ""}`} style={hTouched.email && hErr.email ? iSE : iS} required type="email" placeholder="jane@company.com" value={form.email} onChange={e => setF("email", e.target.value)} onBlur={hBlur("email")} />
                  ...
                  <div>
                    ...
                    <textarea className={`fi${hTouched.message && hErr.message ? " fi-err" : ""}`} style={{ ...(hTouched.message && hErr.message ? iSE : iS), minHeight: isMobile ? 70 : 76, resize: "vertical" as const }} placeholder="Tell us about your app idea..." value={form.message} onChange={e => setF("message", e.target.value)} onBlur={hBlur("message")} />
                    {hTouched.message && <FieldErr msg={hErr.message} />}
                  </div>

                  <button className="btn-teal" type="submit" disabled={loading} style={{ marginTop: 4, opacity: loading ? .6 : 1, cursor: loading ? "wait" : "pointer", width: "100%" }}>
                    {loading ? "Sending…" : "📱 Get Free App Consultation →"}
                  </button>
                  <p style={{ color: "rgba(255,255,255,0.28)", fontSize: isMobile ? 10 : 11, textAlign: "center", margin: 0 }}>🔒 Secure &amp; confidential. No spam, ever.</p>
                </form>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N2} 0%,${N1} 100%)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: isMobile ? 30 : 40, flexWrap: "wrap" }}>
            {CASES.map((c, i) => <button key={i} className={`ss-tab${story === i ? " act" : ""}`} onClick={() => setStory(i)} style={{ padding: isMobile ? "8px 14px" : "10px 20px", fontSize: isMobile ? "12px" : "13.5px" }}><span>{c.icon}</span>{c.industry}</button>)}
          </div>
          <div key={story} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${CASES[story].tagBd}`, borderRadius: 24, overflow: "hidden", animation: "sbFadeUp .45s ease both" }}>
            <div style={{ height: 3, background: `linear-gradient(90deg,transparent,${CASES[story].tagClr},transparent)` }} />
            <div style={{ padding: isMobile ? "20px" : "36px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: isMobile ? 10 : 14 }}>
                {CASES[story].metrics.map((m, i) => (
                  <div key={i} style={{ textAlign: "center", padding: isMobile ? "12px 8px" : "18px 14px", borderRadius: 14, border: "1px solid rgba(255,255,255,0.07)", background: "rgba(255,255,255,0.03)", transition: "transform .25s", cursor: "default" }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; (e.currentTarget as HTMLElement).style.background = "rgba(0,201,167,0.06)"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = ""; (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.03)"; }}>
                    <div style={{ fontSize: isMobile ? 18 : 22, marginBottom: 6 }}>{m.i}</div>
                    <div style={{ fontSize: isMobile ? "20px" : "26px", fontWeight: 900, marginBottom: 4, background: `linear-gradient(135deg,#fff 30%,${T})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{m.v}</div>
          ...
          <div className="svc-grid">
            {SERVICES.map(s => (
              <Link key={s.title} href="" style={{ textDecoration: "none" }}>
                <div className="svc-card" style={{ padding: isMobile ? "20px" : "24px 20px" }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                    <div className="svc-icon" style={{ width: isMobile ? 48 : 52, height: isMobile ? 48 : 52, borderRadius: 14, background: "rgba(0,201,167,0.1)", border: "1px solid rgba(0,201,167,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? 22 : 24, flexShrink: 0, transition: "transform .25s" }}>{s.icon}</div>
                    <span style={{ padding: "3px 10px", borderRadius: 100, fontSize: isMobile ? 9 : 10.5, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: T, background: "rgba(0,201,167,0.08)", border: "1px solid rgba(0,201,167,0.18)" }}>{s.tag}</span>
                  </div>
                  <h3 style={{ fontSize: isMobile ? "15px" : "17px", fontWeight: 700, color: "#fff", lineHeight: 1.3, margin: 0 }}>{s.title}</h3>
                  <p style={{ fontSize: isMobile ? "12px" : "13.5px", color: "rgba(255,255,255,0.5)", lineHeight: 1.7, margin: 0 }}>{s.desc}</p>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N1} 0%,${N2} 100%)`, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: isMobile ? 12 : 16, marginTop: isMobile ? 32 : 48, justifyContent: "center", flexWrap: "wrap" }}>
            <Link href="/hire-crm-developers" className="btn-teal">📱 Hire a Mobile Developer</Link>
            <Link href="/pricing" className="btn-outline">View Pricing →</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N2} 0%,${N1} 100%)`, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div className="two-col-wide">
            <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 12 : 16 }}>
              {AI_FEATS.map((f, i) => (
                <div key={i} className="ai-feat-card" style={{ display: "flex", gap: isMobile ? 12 : 18, alignItems: "flex-start", padding: isMobile ? "16px" : "24px 22px", borderRadius: 16, background: f.clr, border: `1px solid ${f.bd}`, transition: "transform .25s", cursor: "default" }}>
                  <div className="ai-icon-w" style={{ width: isMobile ? 44 : 52, height: isMobile ? 44 : 52, borderRadius: 14, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? 22 : 24, background: f.clr, border: `1px solid ${f.bd}` }}>{f.icon}</div>
                  <div>
                    <h3 style={{ color: "#fff", fontSize: isMobile ? "15px" : "17px", fontWeight: 700, marginBottom: 6, lineHeight: 1.3 }}>{f.title}</h3>
            ...
            <div style={{ position: "relative", borderRadius: 24, overflow: "hidden", background: "linear-gradient(135deg,#0a1f38 0%,#061425 100%)", border: "1px solid rgba(0,201,167,0.15)", minHeight: isMobile ? "auto" : 460, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: isMobile ? "20px" : "36px 32px" }}>
              ...
              <div style={{ position: "relative", zIndex: 2 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                  <div style={{ width: isMobile ? 8 : 10, height: isMobile ? 8 : 10, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 10px #22c55e" }} />
                  <span style={{ color: "rgba(255,255,255,.5)", fontSize: isMobile ? 11 : 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase" as const }}>AI Engine — Live</span>
                </div>
                {[{ label: "Personalisation accuracy", val: "95%", color: T }, { label: "Churn prediction model", val: "91%", color: "#818cf8" }, { label: "Push notification CTR", val: "38%", color: T }, { label: "In-app AI resolution", val: "87%", color: "#f59e0b" }].map((row, i) => (
                  <div key={i} style={{ marginBottom: isMobile ? 10 : 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ color: "rgba(255,255,255,.6)", fontSize: isMobile ? 11 : 12.5, fontWeight: 500 }}>{row.label}</span><span style={{ color: row.color, fontSize: isMobile ? 11 : 12.5, fontWeight: 700 }}>{row.val}</span></div>
                    <div style={{ height: 4, borderRadius: 4, background: "rgba(255,255,255,.06)", overflow: "hidden" }}><div style={{ height: "100%", borderRadius: 4, background: `linear-gradient(90deg,${row.color},${row.color}88)`, width: row.val }} /></div>
                  </div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: isMobile ? 8 : 12, position: "relative", zIndex: 2, marginTop: isMobile ? 16 : 0 }}>
          ...
          <div className="two-col-wide">
            ...
            <div>
              <div style={{ background: "rgba(0,201,167,0.05)", border: "1px solid rgba(0,201,167,0.15)", borderRadius: 24, padding: isMobile ? "30px 20px" : "40px 30px", textAlign: "center" }}>
                <div style={{ fontSize: isMobile ? 48 : 64, marginBottom: 16 }}>📱</div>
                <h3 style={{ color: "#fff", fontSize: isMobile ? "20px" : "24px", fontWeight: 800, marginBottom: 12 }}>Ready to Build Your Mobile App?</h3>
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: isMobile ? "14px" : "16px", lineHeight: 1.7, marginBottom: 24 }}>Join 500+ businesses that have launched successful iOS and Android apps with NNC Digital.</p>
                <Link href="/contact" className="btn-teal" style={{ width: isMobile ? "100%" : "auto" }}>Get Started Today →</Link>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
                {["🇨🇦 Canada", "🇺🇸 USA", "🇬🇧 UK", "🇮🇳 India"].map(b => (
              ...
              <div style={{ display: "flex", gap: isMobile ? 8 : 10, marginTop: 20, width: "100%", justifyContent: "center" }}>
                <Link href="/contact" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: isMobile ? "12px 20px" : "8px 24px", borderRadius: "100px", border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 600, fontSize: isMobile ? "14px" : "13px", fontFamily: "'Poppins',sans-serif", cursor: "pointer", textDecoration: "none", transition: "all .3s ease", width: isMobile ? "100%" : "auto" }}
                  onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 8px 20px rgba(0,201,167,0.35)"; }}
                  onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = "0 4px 12px rgba(0,0,0,0.1)"; }}>
                  <span style={{ fontSize: isMobile ? "16px" : "14px" }}>📅</span>Book a Free App Strategy Call
                </Link>
              </div>
            </div>
          </div>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: isMobile ? 8 : 12, justifyContent: "center", marginBottom: isMobile ? 30 : 40, flexWrap: "wrap" }}>
            {[{ key: "int", label: "North America & UK", icon: "🌎" }, { key: "india", label: "India (HQ)", icon: "🇮🇳" }].map(t => (
              <button key={t.key} onClick={() => setGTab(t.key as "int" | "india")}
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N2} 0%,${N0} 100%)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", position: "relative", zIndex: 2 }}>
          <div style={{ textAlign: "center", marginBottom: isMobile ? 32 : 40 }}>
            <SectionBadge label="Get In Touch" />
            <h2 style={{ fontSize: isMobile ? "clamp(24px,5vw,28px)" : isTablet ? "32px" : "clamp(36px,3.5vw,42px)", fontWeight: 900, color: "#fff", letterSpacing: "-0.02em", lineHeight: 1.2, marginBottom: 12 }}>
              Ready to Build <GradText>Next-Level Custom Digital Solutions?</GradText>
            </h2>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: isMobile ? "13px" : "15px", lineHeight: 1.75, maxWidth: 540, margin: "0 auto" }}>Tell us about your project. We respond within 24 hours with a free consultation and honest advice.</p>
          </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 20, padding: isMobile ? "20px" : "36px" }}>
              {cSubmitted ? (
                ...
                <form onSubmit={handleCSubmit} noValidate>
                  ...
                  <div className="cf-name" style={{ marginBottom: isMobile ? 12 : 16 }}>
                    <div>
                      <label style={{ display: "block", fontSize: isMobile ? 11 : 12.5, fontWeight: 600, color: "rgba(255,255,255,0.5)", marginBottom: 5, textTransform: "uppercase" as const, letterSpacing: "0.04em" }}>Name *</label>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <textarea className={`fi${cTouched.project && cErr.project ? " fi-err" : ""}`} style={{ ...(cTouched.project && cErr.project ? iSLgE : iSLg), minHeight: isMobile ? 80 : 110, resize: "vertical" as const }} placeholder="Describe your project, goals, and timeline..." value={cForm.project} onChange={e => setCF("project", e.target.value)} onBlur={cBlur("project")} />
