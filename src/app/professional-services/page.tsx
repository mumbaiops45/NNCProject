...
export default function ProfessionalServicesIndustryPage() {
  ...
  const handleHeroSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, buildParams({ firstName: parts[0], lastName: parts.slice(1).join(" "), email: hForm.email, phone: `${hForm.dialCode} ${hForm.phone}`, service: hForm.service, message: hForm.message, title: `New Professional Services Enquiry from ${hForm.name}` }));
      setHSubmitted(true);
  ...
  const handleCSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, buildParams({ firstName: parts[0], lastName: parts.slice(1).join(" "), email: cForm.email, phone: `${cForm.dialCode} ${cForm.phone}`, service: "Professional Services CTA", message: cForm.project, title: `New Professional Services CTA from ${cForm.name}` }));
      setCSubmitted(true);
