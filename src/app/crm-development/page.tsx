...
const LOGOS = [
  ...
  "clients11.png","clients12.png","clients13.png","clients14.png","clients15.png",
  "clients16.png","clients17.png","clients18.png",
];
...
const CASES = [
  {
    industry: "Manufacturing", icon: "🏭", tag: "Operations",
    tagClr: "#818cf8", tagBg: "rgba(99,102,241,.15)", tagBd: "rgba(99,102,241,.35)",
  ...
  {
    industry: "Healthcare", icon: "🏥", tag: "Healthcare",
    tagClr: "#22c55e", tagBg: "rgba(34,197,94,.12)", tagBd: "rgba(34,197,94,.3)",
  ...
  {
    industry: "D2C E-Commerce", icon: "🛒", tag: "E-Commerce",
    tagClr: "#00C9A7", tagBg: "rgba(0,201,167,.12)", tagBd: "rgba(0,201,167,.3)",
...
const SERVICES = [
  ...
  { icon: "📱", title: "CRM Mobile App Dev",       desc: "Tailored for businesses in Canada, USA & UK.", tag: "Mobile",     slug: "crm-mobile-app"       },
...
const FAQS = [
  { q: "What is custom CRM software vs off-the-shelf?",           a: "Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process." },
  { q: "What features can be customised?",                         a: "Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process." },
  { q: "How long does CRM development take?",                      a: "Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process." },
  { q: "What does CRM development cost in Canada or UK?",          a: "Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process." },
  { q: "Do you offer post-launch support?",                        a: "Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process." },
];
...
// All keys match {{variable}} names in the branded HTML email template exactly.
function buildParams(data: {
  ...
  return {
    ...
    title:    `New CRM Consultation from ${data.firstName} ${data.lastName}`.trim(),
    reply_to: data.email,
  };
}
...
export default function CRMDevelopmentPage() {
  ...
  const [hErr,  setHErr ] = useState({ firstName:"", lastName:"", phone:"", email:"", message:"" });
  const [hTouched, setHTouched] = useState({ firstName:false, lastName:false, phone:false, email:false, message:false });
  ...
  if (hTouched[k as keyof typeof hTouched]) hValidate(k, value);
};

  const hValidate = (k: string, v: string): boolean => {
    const msgs: Record<string, string> = {
      firstName: vName(v),
      lastName:  v ? vName(v) : "",
      phone:     vPhone(v, hForm.dialCode),
      email:     vEmail(v),
      message:   vMsg(v),
    };
  ...
  const hBlur = (k: string) => () => {
    ...
    hValidate(k, hForm[k as keyof typeof hForm]);
  };

  const hValidateAll = (): boolean => {
    setHTouched({ firstName:true, lastName:true, phone:true, email:true, message:true });
    const e = {
      firstName: vName(hForm.firstName),
      lastName:  hForm.lastName ? vName(hForm.lastName) : "",
      phone:     vPhone(hForm.phone, hForm.dialCode),
      email:     vEmail(hForm.email),
      message:   vMsg(hForm.message),
    };
    setHErr(e);
  ...
  const handleHeroSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!hValidateAll()) {
      document.querySelector(".h-field-err")?.scrollIntoView({ behavior:"smooth", block:"center" });
      return;
    }
    setHLoading(true);
    try {
      await emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, buildParams({
        firstName: hForm.firstName,
        lastName:  hForm.lastName,
        email:     hForm.email,
      ...
      console.error("EmailJS hero error:", err);
 ...
 const setC = (k: string, v: string) => {
  ...
  if (cTouched[k as keyof typeof cTouched]) cValidate(k, value);
};
  const cValidate = (k: string, v: string): boolean => {
    const msgs: Record<string, string> = {
      name:    vName(v),
      phone:   vPhone(v, cForm.dialCode),
      email:   vEmail(v),
      project: vMsg(v),
    };
  ...
  const cBlur = (k: string) => () => {
    ...
    cValidate(k, cForm[k as keyof typeof cForm]);
  };

  const cValidateAll = (): boolean => {
    setCTouched({ name:true, phone:true, email:true, project:true });
    const e = {
      name:    vName(cForm.name),
      phone:   vPhone(cForm.phone, cForm.dialCode),
      email:   vEmail(cForm.email),
      project: vMsg(cForm.project),
    };
    setCErr(e);
    return Object.values(e).every(v => !v);
  };
  ...
  const handleCSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!cValidateAll()) {
      document.querySelector(".c-field-err")?.scrollIntoView({ behavior:"smooth", block:"center" });
      return;
    }
    setCLoading(true);
    try {
  ...
  // ── Responsive styles ──────────────────────────────────────────────────────
  const getSectionPadding = () => isMobile ? "40px 16px" : isTablet ? "60px 32px" : "80px 48px";
  const getHeroPadding    = () => isMobile ? "10px 16px 60px" : isTablet ? "80px 32px 80px" : "90px 48px 80px";

  const iS: React.CSSProperties = {
    width:"100%", padding:isMobile?"10px 12px":"11px 14px", borderRadius:9,
    background:"rgba(255,255,255,0.07)", border:"1px solid rgba(255,255,255,0.14)",
    color:"#fff", fontSize:isMobile?"13px":"13.5px", fontFamily:"'Poppins',sans-serif",
  ...
  const iSErr: React.CSSProperties = { ...iSLg, borderColor:"#ff4444", background:"rgba(255,68,68,.08)" };

  const btnT: React.CSSProperties = {
    display:"inline-block", padding:isMobile?"12px 20px":"13px 28px", borderRadius:10,
    border:"none", background:`linear-gradient(135deg,${T},${TD})`, color:"#000",
    fontWeight:700, fontSize:isMobile?"14px":"15px", fontFamily:"'Poppins',sans-serif",
    cursor:"pointer", textDecoration:"none", transition:"transform .2s,box-shadow .2s",
  };
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .btn-teal,.btn-white { display:inline-flex;align-items:center;justify-content:center;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:700;font-family:'Poppins',sans-serif;cursor:pointer;text-decoration:none;transition:all .2s;min-width:140px; }
        @media(min-width:768px){.btn-teal,.btn-white{padding:14px 32px;font-size:15px;min-width:160px;}}
        @media(max-width:640px){.btn-teal,.btn-white{width:100%;min-width:0;}}
        .btn-teal{background:linear-gradient(135deg,var(--teal),var(--teal-dark));color:#000;border:none;}
        .btn-teal:hover{transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,201,167,.25);}
        .btn-white{background:#fff;color:#0055b3;border:none;}
        ...
        /* form inputs */
        .fi:focus{border-color:rgba(0,201,167,.5) !important;background:rgba(0,201,167,.06) !important;}
        .fi::placeholder{color:rgba(255,255,255,.28);}
        .fi option{background:#0a1f38;color:#fff;}
        .fi-err{border-color:#ff4444 !important;background:rgba(255,68,68,.08) !important;}
        ...
        .badge{display:inline-block;padding:5px 12px;border-radius:100px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.7);font-size:11px;font-weight:600;}
        .badge-soft{background:rgba(0,201,167,.06);border:1px solid rgba(0,201,167,.2);color:var(--teal);}

        .section-h2{font-size:clamp(24px,3vw,40px);font-weight:900;color:#fff;letter-spacing:-0.02em;line-height:1.15;margin-bottom:14px;}
        .grad-text{background:linear-gradient(135deg,var(--teal),#1fd1b5);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}

        .ss-tab{padding:8px 14px;border-radius:100px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.03);color:rgba(255,255,255,.6);font-size:12px;font-weight:600;cursor:pointer;transition:all .22s;display:flex;align-items:center;gap:6px;}
        @media(min-width:768px){.ss-tab{padding:10px 20px;font-size:13.5px;}}
        .ss-tab.act{border-color:rgba(0,201,167,.45);background:rgba(0,201,167,.1);color:var(--teal);}

        .svc-card{position:relative;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:18px;padding:20px 16px;display:flex;flex-direction:column;gap:12px;transition:all .3s;cursor:default;overflow:hidden;}
        @media(min-width:768px){.svc-card{padding:24px 20px;}}
        .svc-card:hover{transform:translateY(-4px);border-color:rgba(0,201,167,.35);background:rgba(0,201,167,.04);box-shadow:0 20px 40px rgba(0,0,0,.4);}

        .pt-card{border-radius:16px;padding:16px 12px;display:flex;flex-direction:column;text-align:center;align-items:center;transition:all .3s;}
        @media(min-width:768px){.pt-card{padding:20px 16px;}}
        .pt-card:hover{transform:translateY(-4px);box-shadow:0 20px 40px rgba(0,0,0,.4);}
        .pt-card__icon{width:48px;height:48px;border-radius:15px;display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:12px;background:rgba(255,255,255,.05);transition:transform .25s;}
        @media(min-width:768px){.pt-card__icon{width:56px;height:56px;font-size:26px;margin-bottom:16px;}}

        .wcu-point{display:flex;align-items:center;gap:12px;padding:12px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.06);background:rgba(255,255,255,.02);transition:all .2s;cursor:default;}
        @media(min-width:768px){.wcu-point{padding:14px 18px;gap:14px;}}
        .wcu-point:hover{border-color:rgba(0,201,167,.25);background:rgba(0,201,167,.04);transform:translateX(6px);}

        .wcu-stat{text-align:center;padding:14px 8px;border-radius:14px;border:1px solid rgba(0,201,167,.15);background:rgba(0,201,167,.05);transition:all .25s;cursor:default;}
        @media(min-width:768px){.wcu-stat{padding:20px 12px;}}
        .wcu-stat:hover{transform:translateY(-4px);background:rgba(0,201,167,.1);}

        .faq-item{border-radius:14px;border:1px solid rgba(255,255,255,.07);background:rgba(255,255,255,.02);overflow:hidden;cursor:pointer;transition:all .25s;}
        .faq-item.fopen{border-color:rgba(0,201,167,.35);background:rgba(0,201,167,.04);}
        .faq-item__q{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 16px;}
        @media(min-width:768px){.faq-item__q{padding:20px 22px;gap:16px;}}
        .faq-item__tag{color:var(--teal);font-size:11px;font-weight:800;background:rgba(0,201,167,.1);border:1px solid rgba(0,201,167,.2);border-radius:8px;padding:3px 8px;flex-shrink:0;}
        @media(min-width:768px){.faq-item__tag{font-size:13px;padding:4px 10px;}}
        .faq-item__text{font-size:13px;font-weight:700;color:rgba(255,255,255,.78);line-height:1.4;}
        @media(min-width:768px){.faq-item__text{font-size:15px;}}
        .faq-item.fopen .faq-item__text{color:#fff;}
        .faq-item__icon{width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:15px;font-weight:700;line-height:1;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);color:rgba(255,255,255,.5);transition:all .25s;flex-shrink:0;}
        @media(min-width:768px){.faq-item__icon{width:30px;height:30px;font-size:17px;}}
        ...
        .faq-item__a p{padding:0 16px 14px 56px;color:rgba(255,255,255,.55);font-size:12px;line-height:1.7;margin:0;}
        @media(min-width:768px){.faq-item__a p{padding:0 22px 22px 76px;font-size:14px;}}

        .gp-tab{padding:8px 16px;border-radius:10px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.03);color:rgba(255,255,255,.55);font-size:12px;font-weight:700;cursor:pointer;transition:all .22s;}
        @media(min-width:768px){.gp-tab{padding:11px 24px;font-size:14px;}}
        .gp-tab.act{border-color:rgba(0,201,167,.5);background:rgba(0,201,167,.12);color:var(--teal);box-shadow:0 4px 20px rgba(0,201,167,.12);}
        .gp-tab:hover{border-color:rgba(0,201,167,.3);color:var(--teal);}

        /* responsive grids */
        .hero-layout{display:grid;grid-template-columns:1fr;gap:40px;align-items:center;max-width:1280px;margin:0 auto;position:relative;z-index:2;width:100%;padding:0 16px;}
        @media(min-width:768px){.hero-layout{grid-template-columns:1fr 460px;gap:40px;padding:0 24px;}}
        .svc-grid{display:grid;grid-template-columns:1fr;gap:16px;}
        @media(min-width:768px){.svc-grid{grid-template-columns:repeat(2,1fr);gap:20px;}}
        ...
        .two-col-wide{display:grid;grid-template-columns:1fr;gap:32px;align-items:start;}
        @media(min-width:768px){.two-col-wide{grid-template-columns:1fr 1fr;gap:40px;}}
        .wcu-stats{display:grid;grid-template-columns:repeat(2,1fr);gap:12px;}
        @media(min-width:768px){.wcu-stats{grid-template-columns:repeat(4,1fr);}}
        .cf-grid{display:grid;grid-template-columns:1fr;gap:32px;}
        @media(min-width:1024px){.cf-grid{grid-template-columns:1fr 1fr;gap:40px;}}
        .cf-name{display:grid;grid-template-columns:1fr;gap:12px;}
        @media(min-width:768px){.cf-name{grid-template-columns:1fr 1fr;gap:16px;}}
        .phone-row{display:flex;flex-direction:column;gap:8px;}
        @media(min-width:768px){.phone-row{flex-direction:row;gap:8px;}}
      ...
      <div style={{ fontFamily:"'Poppins',sans-serif" }}>
        ...
        <section style={{ padding:getHeroPadding(), background:`linear-gradient(135deg,${N0} 0%,#041628 45%,${N0} 100%)`, position:"relative", overflow:"hidden", minHeight:isMobile?"auto":"90vh", display:"flex", alignItems:"center" }}>
          ...
          <div className="hero-layout">
            ...
            {/* Right — Hero Form */}
            <div style={{ background:"rgba(8,20,40,0.85)", border:"1px solid rgba(0,201,167,0.22)", borderRadius:20, padding:isMobile?"24px 20px":"32px 28px", backdropFilter:"blur(16px)", boxShadow:"0 32px 80px rgba(0,0,0,.5),0 0 40px rgba(0,201,167,.06)", position:"relative", overflow:"hidden" }}>
              ...
              {hSubmitted ? (
                <div style={{ textAlign:"center", padding:isMobile?"20px 0":"40px 0" }}>
                  ...
                  <button onClick={() => setHSubmitted(false)} style={{ padding:isMobile?"10px 22px":"11px 26px", borderRadius:9, border:"none", background:`linear-gradient(135deg,${T},${TD})`, color:"#000", fontWeight:700, fontSize:isMobile?13:14, fontFamily:"'Poppins',sans-serif", cursor:"pointer" }}>Send Another →</button>
                </div>
              ) : (
                <>
                  ...
                  <form onSubmit={handleHeroSubmit} noValidate style={{ display:"flex", flexDirection:"column", gap:isMobile?10:12 }}>
                    ...
                    <div className="cf-name">
                      <div>
                        <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>First Name *</p>
                        <input className={`fi${hTouched.firstName && hErr.firstName ? " fi-err" : ""}`} style={iS} placeholder="Jane" value={hForm.firstName} onChange={e => setH("firstName", e.target.value)} onBlur={hBlur("firstName")} />
                        {hTouched.firstName && <FieldErr msg={hErr.firstName} />}
                      </div>
                      <div>
                        <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Last Name</p>
                        <input className="fi" style={iS} placeholder="Smith" value={hForm.lastName} onChange={e => setH("lastName", e.target.value)} onBlur={hBlur("lastName")} />
                        {hTouched.lastName && <FieldErr msg={hErr.lastName} />}
                      </div>
                    </div>

                    {/* Phone */}
                    <div>
                      <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Phone *</p>
                      <div className="phone-row">
                        <select className="fi" style={{ width:isMobile?"100%":"82px" }} value={hForm.dialCode} onChange={e => setH("dialCode", e.target.value)}>
                          {DIAL_CODES.map((d,i) => <option key={i} value={d.code}>{d.flag} {d.code}</option>)}
                        </select>
                        <input className={`fi${hTouched.phone && hErr.phone ? " fi-err" : ""}`} style={{ ...iS, flex:1 }} type="tel" placeholder="647 XXX XXXX" value={hForm.phone} onChange={e => setH("phone", e.target.value)} onBlur={hBlur("phone")} />
                      </div>
                    ...
                    {/* Email */}
                    <div>
                      <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Business Email *</p>
                      <input className={`fi${hTouched.email && hErr.email ? " fi-err" : ""}`} style={iS} type="email" placeholder="jane@company.com" value={hForm.email} onChange={e => setH("email", e.target.value)} onBlur={hBlur("email")} />
                      {hTouched.email && <FieldErr msg={hErr.email} />}
                    </div>

                    {/* Service */}
                    <div>
                      <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Service</p>
                    ...
                    {/* Message */}
                    <div>
                      <p style={{ fontSize:isMobile?10:11, fontWeight:600, color:"rgba(255,255,255,.45)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Message *</p>
                      <textarea className={`fi${hTouched.message && hErr.message ? " fi-err" : ""}`} style={{ ...iS, minHeight:isMobile?70:76, resize:"vertical" }} placeholder="Tell us about your project..." value={hForm.message} onChange={e => setH("message", e.target.value)} onBlur={hBlur("message")} />
                      {hTouched.message && <FieldErr msg={hErr.message} />}
                    </div>

                    <button type="submit" disabled={hLoading} style={{ ...btnT, width:"100%", marginTop:4, opacity:hLoading?.6:1, cursor:hLoading?"wait":"pointer" }}>
            ...
            <div key={ri} style={{ overflow:"hidden", marginBottom:ri<2?(isMobile?16:20):0 }}>
              <div className="cl-track" style={{ animation:`${row.dir} ${row.dur} linear infinite` }}>
                {[...row.logos,...row.logos].map((logo,i) => (
                  <div key={i} style={{ flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", height:isMobile?60:70, padding:isMobile?"8px 14px":"10px 18px", background:"#fff", borderRadius:10, margin:"0 8px", boxShadow:"0 6px 20px rgba(0,0,0,.15)", opacity:.9, transition:"transform .3s,box-shadow .3s" }}
                    onMouseEnter={e => { const el=e.currentTarget as HTMLElement; el.style.transform="scale(1.08)"; el.style.boxShadow="0 10px 28px rgba(0,0,0,.25)"; }}
                    onMouseLeave={e => { const el=e.currentTarget as HTMLElement; el.style.transform=""; el.style.boxShadow="0 6px 20px rgba(0,0,0,.15)"; }}>
        ...
        <section style={{ padding:getSectionPadding(), background:`linear-gradient(180deg,${N2} 0%,${N1} 100%)`, position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ maxWidth:1200, margin:"0 auto", position:"relative", zIndex:2 }}>
            <div style={{ textAlign:"center", marginBottom:isMobile?32:48 }}>
              ...
              <p style={{ color:"rgba(255,255,255,.45)", fontSize:isMobile?13:14, lineHeight:1.7, maxWidth:500, margin:"0 auto" }}>Real results for real businesses across Canada, USA &amp; UK.</p>
            </div>
            <div style={{ display:"flex", gap:10, justifyContent:"center", marginBottom:isMobile?30:40, flexWrap:"wrap" }}>
              {CASES.map((c,i) => <button key={i} className={`ss-tab${story===i?" act":""}`} onClick={() => setStory(i)}><span>{c.icon}</span>{c.industry}</button>)}
            </div>
              ...
              <div style={{ padding:isMobile?"20px":"36px" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:isMobile?10:14 }}>
                  {CASES[story].metrics.map((m,i) => (
                    <div key={i} style={{ textAlign:"center", padding:isMobile?"12px 8px":"18px 14px", borderRadius:14, border:"1px solid rgba(255,255,255,.07)", background:"rgba(255,255,255,.03)" }}>
                      ...
                      <div style={{ color:"rgba(255,255,255,.45)", fontSize:isMobile?10:11 }}>{m.l}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:8, marginTop:24 }}>
              {CASES.map((_,i) => <button key={i} onClick={() => setStory(i)} style={{ width:story===i?24:8, height:8, borderRadius:100, background:story===i?T:"rgba(255,255,255,.2)", border:"none", cursor:"pointer", transition:"all .3s ease" }} />)}
            </div>
          </div>
        </section>
        ...
        <section style={{ padding:getSectionPadding(), background:`linear-gradient(180deg,${N1} 0%,${N2} 60%,${N1} 100%)`, position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:2 }}>
            <div style={{ textAlign:"center", marginBottom:isMobile?36:48 }}>
              ...
              <p style={{ color:"rgba(255,255,255,.5)", fontSize:isMobile?13:14, lineHeight:1.75, maxWidth:580, margin:"0 auto" }}>A comprehensive lineup of CRM solutions for clients across Canada, USA &amp; UK.</p>
            </div>
                ...
                <Link key={s.title} href="" style={{ textDecoration:"none" }}>
                  <div className="svc-card">
                    <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
                      <div style={{ width:isMobile?48:52, height:isMobile?48:52, borderRadius:14, background:"rgba(0,201,167,.1)", border:"1px solid rgba(0,201,167,.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:isMobile?22:24, flexShrink:0 }}>{s.icon}</div>
                      <span style={{ padding:"3px 10px", borderRadius:100, fontSize:isMobile?9:10.5, fontWeight:600, letterSpacing:"0.08em", textTransform:"uppercase", color:T, background:"rgba(0,201,167,.08)", border:"1px solid rgba(0,201,167,.18)" }}>{s.tag}</span>
                    </div>
                    <h3 style={{ fontSize:isMobile?"15px":"17px", fontWeight:700, color:"#fff", lineHeight:1.3, margin:0 }}>{s.title}</h3>
                    <p style={{ fontSize:isMobile?"12px":"13.5px", color:"rgba(255,255,255,.5)", lineHeight:1.7, margin:0 }}>{s.desc}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section style={{ padding:getSectionPadding(), background:N1, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)", width:isMobile?400:600, height:isMobile?300:400, borderRadius:"50%", background:"radial-gradient(ellipse,rgba(0,201,167,.05) 0%,transparent 65%)", pointerEvents:"none", zIndex:0 }} />
          <div style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:2 }}>
            <div style={{ textAlign:"center", marginBottom:isMobile?36:48 }}>
            ...
            <div className="pt-grid">
              {TOOLS.map((tool,i) => (
                <div key={i} className="pt-card" style={{ background:tool.bg, border:`1px solid ${tool.bd}` }}>
                  ...
                  <p style={{ color:"rgba(255,255,255,.5)", fontSize:isMobile?"11px":"12.5px", lineHeight:1.6, margin:0 }}>{tool.purpose}</p>
        ...
        <section style={{ padding:getSectionPadding(), background:`linear-gradient(180deg,${N1} 0%,${N2} 100%)`, position:"relative", overflow:"hidden" }}>
          <div style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:2 }}>
            ...
            <div style={{ display:"flex", gap:isMobile?12:16, marginTop:isMobile?32:48, justifyContent:"center", flexWrap:"wrap" }}>
              <Link href="/hire-crm-developers" style={btnT}>📅 Hire a CRM Developer</Link>
              <Link href="/pricing" style={{ ...btnT, background:"transparent", border:"1.5px solid rgba(0,201,167,.35)", color:T }}>View Pricing →</Link>
            </div>
          </div>
        </section>
        ...
        <section style={{ padding:getSectionPadding(), background:`linear-gradient(180deg,${N2} 0%,${N1} 100%)`, position:"relative", overflow:"hidden" }}>
          <div style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:2 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":isTablet?"repeat(2,1fr)":"repeat(4,1fr)", gap:isMobile?16:20 }}>
              {AI_FEATS.map((f,i) => (
                <div key={i} style={{ background:"rgba(0,201,167,.05)", border:"1px solid rgba(0,201,167,.15)", borderRadius:16, padding:isMobile?"20px 16px":"28px 20px", textAlign:"center" }}>
                  ...
                  <p style={{ color:"rgba(255,255,255,.52)", fontSize:isMobile?"11px":"13px", lineHeight:1.6, margin:0 }}>{f.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background:"linear-gradient(135deg,#0055b3 0%,#0077cc 35%,#0055b3 100%)", backgroundSize:"300% 300%", animation:"ctaBgShift 8s ease infinite", padding:isMobile?"60px 20px":"80px 48px", textAlign:"center", position:"relative" }}>
            <div style={{ position:"relative", zIndex:2, maxWidth:isMobile?"100%":800, margin:"0 auto" }}>
              <h2 style={{ fontSize:isMobile?"clamp(22px,6vw,28px)":isTablet?"32px":"clamp(36px,4vw,48px)", fontWeight:800, color:"#fff", lineHeight:1.2, letterSpacing:"-0.02em", marginBottom:16 }}>
                Want <span style={{ textDecoration:"underline", textDecorationColor:"rgba(255,255,255,.4)" }}>CRM Development solutions</span> that take your business to the next level?
              </h2>
              <p style={{ color:"rgba(255,255,255,.9)", fontSize:isMobile?"16px":"18px", lineHeight:1.6, marginBottom:isMobile?28:36, fontWeight:500 }}>Connect with NNC Digital today.</p>
          ...
          <div style={{ maxWidth:1280, margin:"0 auto", position:"relative", zIndex:2 }}>
            <div className="two-col-wide">
              <div>
                ...
                <p style={{ color:"rgba(255,255,255,.55)", fontSize:isMobile?"13px":"15px", lineHeight:1.8, marginBottom:16 }}>Backed by <span style={{ color:"#fff", fontWeight:600 }}>Nakshatra Namaha Creations Pvt. Ltd.</span> — one of Bangalore&apos;s most respected digital studios with <span style={{ color:T, fontWeight:600 }}>8+ years of experience</span> and <span style={{ color:T, fontWeight:600 }}>565+ projects delivered</span>.</p>
              ...
              <div>
                <div style={{ background:"rgba(0,201,167,.05)", border:"1px solid rgba(0,201,167,.15)", borderRadius:24, padding:isMobile?"30px 20px":"40px 30px", textAlign:"center" }}>
                  ...
                  <p style={{ color:"rgba(255,255,255,.6)", fontSize:isMobile?"14px":"16px", lineHeight:1.7, marginBottom:24 }}>Join 1500+ businesses that have streamlined their operations with our custom CRM solutions.</p>
          ...
          <div style={{ maxWidth:1180, margin:"0 auto", position:"relative", zIndex:2 }}>
            ...
            <div style={{ display:"flex", gap:isMobile?8:12, justifyContent:"center", marginBottom:isMobile?30:40, flexWrap:"wrap" }}>
              {[{key:"int",label:"North America & UK",icon:"🌎"},{key:"india",label:"India (HQ)",icon:"🇮🇳"}].map(t => (
                <button key={t.key} onClick={() => setGTab(t.key as "int"|"india")} style={{ padding:isMobile?"12px 24px":"14px 32px", borderRadius:50, border:"none", background:gTab===t.key?`linear-gradient(135deg,${T},${TD})`:"rgba(255,255,255,.05)", color:gTab===t.key?"#000":"#fff", fontSize:isMobile?14:16, fontWeight:600, cursor:"pointer", transition:"all .3s ease", boxShadow:gTab===t.key?`0 8px 20px ${T}40`:"none", display:"flex", alignItems:"center", gap:8 }}>
        ...
        <section style={{ padding:getSectionPadding(), background:`linear-gradient(180deg,${N2} 0%,${N0} 100%)`, position:"relative", overflow:"hidden" }}>
          <div style={{ maxWidth:1180, margin:"0 auto", position:"relative", zIndex:2 }}>
            <div style={{ textAlign:"center", marginBottom:isMobile?32:40 }}>
              ...
              <p style={{ color:"rgba(255,255,255,.45)", fontSize:isMobile?"13px":"15px", lineHeight:1.75, maxWidth:540, margin:"0 auto" }}>Tell us about your project. We respond within 24 hours with a free consultation and honest advice.</p>
            </div>
            ...
            <div className="cf-grid">
              ...
              <div style={{ background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.08)", borderRadius:20, padding:isMobile?"20px":"36px" }}>
                {cSubmitted ? (
                  <div style={{ textAlign:"center", padding:isMobile?"20px 0":"40px 0" }}>
                    ...
                    <button onClick={() => setCSubmitted(false)} style={{ ...btnT, minWidth:"auto", padding:isMobile?"10px 22px":"12px 28px" }}>Send Another →</button>
                  </div>
                ) : (
                  <form onSubmit={handleCSubmit} noValidate>
                    ...
                    <div className="cf-name" style={{ marginBottom:isMobile?12:16 }}>
                      <div>
                        <p style={{ fontSize:isMobile?11:12.5, fontWeight:600, color:"rgba(255,255,255,.5)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Name *</p>
                        <input className={`fi${cTouched.name && cErr.name ? " fi-err" : ""}`} style={iSLg} placeholder="Jane Smith" value={cForm.name} onChange={e => setC("name", e.target.value)} onBlur={cBlur("name")} />
                        {cTouched.name && <FieldErr msg={cErr.name} />}
                      </div>
                      <div>
                        <p style={{ fontSize:isMobile?11:12.5, fontWeight:600, color:"rgba(255,255,255,.5)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Phone *</p>
                        <div className="phone-row">
                          <select className="fi" style={{ width:isMobile?"100%":"90px", padding:isMobile?"12px 14px":"13px 8px" }} value={cForm.dialCode} onChange={e => setC("dialCode", e.target.value)}>
                            {DIAL_CODES.map((d,i) => <option key={i} value={d.code}>{d.flag} {d.code}</option>)}
                          </select>
                          <input className={`fi${cTouched.phone && cErr.phone ? " fi-err" : ""}`} style={iSLg} type="tel" placeholder="647 XXX XXXX" value={cForm.phone} onChange={e => setC("phone", e.target.value)} onBlur={cBlur("phone")} />
                        </div>
                    ...
                    <div style={{ marginBottom:isMobile?12:16 }}>
                      <p style={{ fontSize:isMobile?11:12.5, fontWeight:600, color:"rgba(255,255,255,.5)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Business Email *</p>
                      <input className={`fi${cTouched.email && cErr.email ? " fi-err" : ""}`} style={iSLg} type="email" placeholder="jane@yourcompany.com" value={cForm.email} onChange={e => setC("email", e.target.value)} onBlur={cBlur("email")} />
                      {cTouched.email && <FieldErr msg={cErr.email} />}
                    </div>

                    {/* Service */}
                    <div style={{ marginBottom:isMobile?12:16 }}>
                      <p style={{ fontSize:isMobile?11:12.5, fontWeight:600, color:"rgba(255,255,255,.5)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Service of Interest</p>
                    ...
                    <div style={{ marginBottom:20 }}>
                      <p style={{ fontSize:isMobile?11:12.5, fontWeight:600, color:"rgba(255,255,255,.5)", marginBottom:5, textTransform:"uppercase", letterSpacing:"0.04em" }}>Project Description *</p>
                      <textarea className={`fi${cTouched.project && cErr.project ? " fi-err" : ""}`} style={{ ...iSLg, minHeight:isMobile?80:110, resize:"vertical" }} placeholder="Briefly describe your project, goals, and timeline..." value={cForm.project} onChange={e => setC("project", e.target.value)} onBlur={cBlur("project")} />
                      {cTouched.project && <FieldErr msg={cErr.project} />}
                    </div>

                    <button className="btn-teal" type="submit" disabled={cLoading} style={{ width:"100%", padding:"15px", opacity:cLoading?.6:1, cursor:cLoading?"wait":"pointer" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:isMobile?16:20 }}>
                <div style={{ background:"rgba(0,201,167,.05)", border:"1px solid rgba(0,201,167,.18)", borderRadius:18, padding:isMobile?"20px":"28px" }}>
                  ...
                  {[
                    {s:"1",text:"We review your project within a few hours — no bots."},
                    {s:"2",text:"A senior consultant calls you within 24 hours."},
                    {s:"3",text:"We send a free scoping document with timeline & cost."},
                    {s:"4",text:"You decide — no pressure, no obligation."},
                  ].map((step,i) => (
                ...
                <div style={{ background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.08)", borderRadius:18, padding:isMobile?"20px":"26px" }}>
                  ...
                  {[{flag:"🇺🇸",label:"USA",phone:"+91 9900566466"},{flag:"🇨🇦",label:"Canada",phone:"+91 9900566466"},{flag:"🇬🇧",label:"UK",phone:"+91 9900566466"}].map((c,i) => (
