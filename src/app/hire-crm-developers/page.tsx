...
const TOOLS = [
  ...
  { icon: "🐘", name: "Custom CRM (Laravel/PHP)", purpose: "Best-in-class for performance.", clr: "rgba(255,45,32,.08)", bd: "rgba(255,45,32,.22)" },
  { icon: "💬", name: "WhatsApp API", purpose: "Enterprise-grade integration.", clr: "rgba(37,211,102,.08)", bd: "rgba(37,211,102,.22)" },
  { icon: "🔌", name: "REST & GraphQL", purpose: "Best-in-class API integration.", clr: "rgba(226,77,141,.08)", bd: "rgba(226,77,141,.22)" },
];
...
function buildParams(data: { firstName: string; lastName: string; email: string; phone: string; service: string; message: string }) {
  return {
    ...
    title:           `New CRM Developer Enquiry from ${data.firstName} ${data.lastName}`.trim(),
    reply_to:        data.email,
  };
}
...
export default function HireCRMDevelopersPage() {
  ...
  // ── Responsive ───────────────────────────────────────────────────────────────
  const getSectionPadding = () => { if (isMobile) return "20px 20px"; if (isTablet) return "80px 32px"; return "20px 48px"; };
  const getHeroPadding    = () => { if (isMobile) return "2px 20px 60px"; if (isTablet) return "80px 32px 70px"; return "90px 48px 80px"; };
  ...
  const taS: React.CSSProperties = { ...iSLg, minHeight: isMobile ? 90 : 110, resize: "vertical" as const };
  ...
  return (
    <>
      ...
      <style jsx global>{`
        ...
        .btn-teal { transition: all 0.3s ease; cursor: pointer; }
        .btn-teal:hover { transform: translateY(-3px); box-shadow: 0 12px 36px rgba(0,201,167,0.5); }
        .cl-track { display: flex; gap: 40px; animation: marquee 30s linear infinite; width: fit-content; }
        .cl-track:hover { animation-play-state: paused; }
        ...
        .svc-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; padding: 28px 24px; display: flex; flex-direction: column; gap: 16px; transition: all 0.3s ease; animation: sbFadeUp 0.5s ease both; }
        .svc-card:hover { transform: translateY(-6px); border-color: ${T}; box-shadow: 0 20px 40px rgba(0,0,0,0.3); }
        .kb-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 20px; }
        @media (max-width: 960px) { .kb-grid { grid-template-columns: repeat(2,1fr) !important; } }
        @media (max-width: 640px) { .kb-grid { grid-template-columns: 1fr !important; } }
        .kb-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; padding: 24px; display: flex; gap: 16px; transition: all 0.3s ease; }
        .kb-card:hover { transform: translateY(-4px); border-color: ${T}; }
        .pt-grid { display: grid; grid-template-columns: repeat(5,1fr); gap: 16px; }
        @media (max-width: 1024px) { .pt-grid { grid-template-columns: repeat(4,1fr) !important; } }
        @media (max-width: 768px)  { .pt-grid { grid-template-columns: repeat(3,1fr) !important; } }
        @media (max-width: 480px)  { .pt-grid { grid-template-columns: repeat(2,1fr) !important; } }
        .pt-card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); border-radius: 16px; padding: 20px 16px; text-align: center; transition: all 0.3s ease; }
        .pt-card:hover { transform: translateY(-4px); border-color: ${T}; }
        .two-col { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
        @media (max-width: 768px) { .two-col { grid-template-columns: 1fr !important; gap: 16px !important; } }
        .two-col-wide { display: grid; grid-template-columns: 1fr 1fr; gap: 48px; align-items: center; }
        @media (max-width: 960px) { .two-col-wide { grid-template-columns: 1fr !important; gap: 40px !important; } }
        .ai-feat-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 20px; }
        @media (max-width: 960px) { .ai-feat-grid { grid-template-columns: repeat(2,1fr) !important; } }
        @media (max-width: 480px)  { .ai-feat-grid { grid-template-columns: 1fr !important; } }
        .hero-layout { max-width: 1280px; margin: 0 auto; width: 100%; display: grid; grid-template-columns: 1fr 1fr; gap: 60px; align-items: center; position: relative; z-index: 3; padding: 0 24px; }
        @media (max-width: 768px) { .hero-layout { grid-template-columns: 1fr !important; gap: 40px !important; } }
        .wcu-point { display: flex; align-items: center; gap: 14px; padding: 14px 18px; border-radius: 12px; border: 1px solid rgba(255,255,255,0.06); background: rgba(255,255,255,0.02); transition: all 0.2s; }
        .wcu-point:hover { border-color: ${T}; background: rgba(0,201,167,0.05); }
        .wcu-stats { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; }
        @media (max-width: 640px) { .wcu-stats { grid-template-columns: repeat(2,1fr) !important; } }
        .wcu-stat { text-align: center; padding: 22px 14px; border-radius: 14px; border: 1px solid rgba(0,201,167,0.15); background: rgba(0,201,167,0.05); transition: all 0.25s; }
        .wcu-stat:hover { transform: translateY(-4px); background: rgba(0,201,167,0.12); }
        .faq-item { border: 1px solid rgba(255,255,255,0.08); border-radius: 16px; background: rgba(255,255,255,0.02); overflow: hidden; cursor: pointer; transition: all 0.25s ease; }
        .faq-item.fopen { border-color: rgba(0,201,167,0.4); background: rgba(0,201,167,0.06); }
        .ss-tab { padding: 8px 18px; border-radius: 100px; border: 1px solid rgba(255,255,255,0.1); background: transparent; color: rgba(255,255,255,0.6); font-size: 13px; font-weight: 600; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; gap: 6px; }
        .ss-tab.act { background: rgba(0,201,167,0.15); border-color: ${T}; color: ${T}; }
        .gp-tab { padding: 11px 24px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1); background: rgba(255,255,255,0.03); color: rgba(255,255,255,0.55); font-size: 14px; font-weight: 700; cursor: pointer; transition: all 0.22s; }
        .gp-tab:hover { border-color: ${T}50; color: ${T}; }
        .gp-tab.act { border-color: ${T}80; background: rgba(0,201,167,0.12); color: ${T}; box-shadow: 0 4px 20px rgba(0,201,167,0.12); }
        .cf-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 32px; }
        @media (max-width: 768px) { .cf-grid { grid-template-columns: 1fr !important; gap: 24px !important; } }
        .cf-name { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        @media (max-width: 480px) { .cf-name { grid-template-columns: 1fr !important; gap: 8px !important; } }
              ...
              <>
                ...
                <form onSubmit={handleHeroSubmit} noValidate style={{ display: "flex", flexDirection: "column", gap: isMobile ? 10 : 12 }}>
                  ...
                  <div>
                    ...
                    <input className={`fi${hTouched.email && hErr.email ? " fi-err" : ""}`} style={hTouched.email && hErr.email ? iSE : iS} required type="email" placeholder="jane@company.com" value={hForm.email} onChange={e => setH("email", e.target.value)} onBlur={hBlur("email")} />
                  ...
                  <div>
                    ...
                    <textarea className={`fi${hTouched.message && hErr.message ? " fi-err" : ""}`} style={{ ...(hTouched.message && hErr.message ? { ...iS, borderColor: "#ff4444", background: "rgba(255,68,68,.08)" } : iS), minHeight: isMobile ? 70 : 76, resize: "vertical" as const }} placeholder="Tell us about your CRM platform and developer requirements..." value={hForm.message} onChange={e => setH("message", e.target.value)} onBlur={hBlur("message")} />
                    {hTouched.message && <FieldErr msg={hErr.message} />}
                  </div>
                  <button className="btn-teal" type="submit" disabled={hLoading} style={{ padding: isMobile ? "12px" : "13px", borderRadius: 10, border: "none", marginTop: 4, background: hLoading ? "rgba(0,201,167,0.5)" : `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 800, fontSize: isMobile ? "13px" : "14.5px", fontFamily: "'Poppins',sans-serif", cursor: hLoading ? "wait" : "pointer", opacity: hLoading ? 0.7 : 1 }}>
                    {hLoading ? "Sending…" : "👨‍💻 Get Free Developer Match →"}
                  </button>
                  <p style={{ color: "rgba(255,255,255,0.28)", fontSize: isMobile ? 10 : 11, textAlign: "center", margin: 0 }}>🔒 Secure &amp; confidential. No spam, ever.</p>
                </form>
              </>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N2} 0%,${N1} 100%)`, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ textAlign: "center", marginTop: 36 }}>
            <Link href="/case-studies"><button className="btn-teal" style={{ padding: isMobile ? "12px 28px" : "14px 36px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer" }}>View All Case Studies →</button></Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N1} 0%,${N2} 100%)`, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ textAlign: "center", marginTop: 48 }}>
            <Link href=""><button className="btn-teal" style={{ padding: isMobile ? "12px 28px" : "14px 36px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer" }}>Find Your Developer →</button></Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N1} 0%,${N2} 100%)`, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: isMobile ? 12 : 16, marginTop: 40, justifyContent: "center", flexWrap: "wrap" }}>
            {/* <Link href="/hire-developers"><button className="btn-teal" style={{ width: isMobile ? 210 : 260, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "12px 0" : "14px 0", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer" }}>👨‍💻 Hire a CRM Developer</button></Link> */}
            <Link href="/pricing"><button style={{ width: isMobile ? 210 : 260, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "12px 0" : "14px 0", borderRadius: 10, border: "1.5px solid rgba(0,201,167,0.35)", background: "transparent", color: T, fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer", transition: "border-color .2s,background .2s" }} onMouseEnter={e => { e.currentTarget.style.borderColor = T; e.currentTarget.style.background = "rgba(0,201,167,0.07)"; }} onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(0,201,167,0.35)"; e.currentTarget.style.background = "transparent"; }}>View Pricing →</button></Link>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1280, margin: "0 auto", position: "relative", zIndex: 2 }}>
          <div className="two-col-wide">
            <div>
              ...
              <p style={{ color: "rgba(255,255,255,0.55)", fontSize: isMobile ? "13px" : "15px", lineHeight: 1.8, marginBottom: 16 }}>Backed by <span style={{ color: "#fff", fontWeight: 600 }}>Nakshatra Namaha Creations Pvt. Ltd.</span> — one of Bangalore's most respected digital studios with <span style={{ color: T, fontWeight: 600 }}>8+ years of experience</span> and <span style={{ color: T, fontWeight: 600 }}>565+ projects delivered</span>.</p>
              <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 8 : 10, marginBottom: 30 }}>
                {WCU_POINTS.map((p, i) => (<div key={i} className="wcu-point" style={{ padding: isMobile ? "12px 14px" : "14px 18px" }}><span style={{ fontSize: isMobile ? 16 : 18, flexShrink: 0 }}>{p.icon}</span><span style={{ color: "rgba(255,255,255,0.72)", fontSize: isMobile ? "12px" : "14px", fontWeight: 500 }}>{p.text}</span></div>))}
              </div>
              <div className="wcu-stats">
                {WCU_STATS.map(st => (<div key={st.l} className="wcu-stat" style={{ padding: isMobile ? "16px 10px" : "22px 14px" }}><div style={{ fontSize: isMobile ? "18px" : "24px", fontWeight: 900, marginBottom: 2, background: `linear-gradient(135deg,#fff 30%,${T})`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}><Counter to={st.n} sfx={st.s} /></div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: isMobile ? "10px" : "11.5px", fontWeight: 500 }}>{st.l}</div></div>))}
              </div>
            </div>
            <div>
              <div style={{ background: "rgba(0,201,167,0.05)", border: "1px solid rgba(0,201,167,0.15)", borderRadius: 24, padding: isMobile ? "30px 20px" : "40px 30px", textAlign: "center" }}>
                <div style={{ fontSize: isMobile ? 48 : 64, marginBottom: 16 }}>🚀</div>
                <h3 style={{ color: "#fff", fontSize: isMobile ? "20px" : "24px", fontWeight: 800, marginBottom: 12 }}>Start Your CRM Journey Today</h3>
                <p style={{ color: "rgba(255,255,255,0.6)", fontSize: isMobile ? "14px" : "16px", lineHeight: 1.7, marginBottom: 24 }}>Join 1500+ businesses that have transformed their operations with our dedicated CRM developers.</p>
                <Link href="/hire-developers"><button className="btn-teal" style={{ padding: isMobile ? "12px 28px" : "14px 36px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer" }}>Get Started Today →</button></Link>
              </div>
              <div style={{ display: "flex", gap: 10, marginTop: 20, flexWrap: "wrap" }}>
                {["🇨🇦 Canada", "🇺🇸 USA", "🇬🇧 UK", "🇮🇳 India"].map(b => (<span key={b} style={{ padding: isMobile ? "5px 12px" : "6px 14px", borderRadius: 100, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)", color: "rgba(255,255,255,0.6)", fontSize: isMobile ? "11px" : "12.5px", fontWeight: 500 }}>{b}</span>))}
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: getSectionPadding(), background: `linear-gradient(180deg,${N1} 0%,${N2} 100%)`, position: "relative", overflow: "hidden" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 10 : 12 }}>
            {[
              { q: "How quickly can a developer start?", n: "Q1" },
              { q: "What CRM platforms do your developers work with?", n: "Q2" },
              { q: "Can I trial a developer before committing?", n: "Q3" },
              { q: "What does a dedicated CRM developer cost per month?", n: "Q4" },
            ].map((item, index) => (
              <div key={index} className={`faq-item${faq === index ? " fopen" : ""}`} onClick={() => setFaq(faq === index ? null : index)}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: isMobile ? "14px 16px" : "20px 22px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 10 : 14 }}>
                    <span style={{ color: T, fontSize: isMobile ? 12 : 13, fontWeight: 800, background: "rgba(0,201,167,0.1)", border: "1px solid rgba(0,201,167,0.2)", borderRadius: 8, padding: isMobile ? "3px 8px" : "4px 10px", flexShrink: 0 }}>{item.n}</span>
                    <span style={{ fontSize: isMobile ? "13px" : "15px", fontWeight: 700, color: faq === index ? "#fff" : "rgba(255,255,255,0.78)", lineHeight: 1.4 }}>{item.q}</span>
                  </div>
                  <div style={{ width: isMobile ? 26 : 30, height: isMobile ? 26 : 30, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: isMobile ? 15 : 17, fontWeight: 700, background: faq === index ? T : "rgba(255,255,255,0.07)", border: `1px solid ${faq === index ? T : "rgba(255,255,255,0.12)"}`, color: faq === index ? "#000" : "rgba(255,255,255,0.5)", transform: faq === index ? "rotate(45deg)" : "rotate(0deg)", transition: "all .25s ease" }}>+</div>
                </div>
                <div style={{ maxHeight: faq === index ? 500 : 0, overflow: "hidden", transition: "max-height .38s ease" }}>
                  <p style={{ padding: isMobile ? "0 16px 14px 54px" : "0 22px 22px 80px", color: "rgba(255,255,255,0.55)", fontSize: isMobile ? "12px" : "14px", lineHeight: 1.7, margin: 0 }}>
                    Detailed answer tailored for businesses in Canada, USA, and UK — covering timeline, cost, compliance, and process.
                  </p>
                </div>
          ...
          <div style={{ textAlign: "center", marginTop: 40 }}>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: isMobile ? "12px" : "14px", marginBottom: 16 }}>Still have questions? We respond within 24 hours.</p>
            <Link href="/contact"><button className="btn-teal" style={{ padding: isMobile ? "12px 28px" : "13px 32px", borderRadius: 10, border: "none", background: `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 700, fontSize: isMobile ? "13px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: "pointer" }}>Ask Us Anything →</button></Link>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1180, margin: "0 auto", position: "relative", zIndex: 2 }}>
          ...
          <div className="cf-grid">
              ...
              {cSubmitted ? (
                ...
                <form onSubmit={handleCSubmit} noValidate>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    ...
                    <textarea className={`fi${cTouched.project && cErr.project ? " fi-err" : ""}`} style={cTouched.project && cErr.project ? taErr : taS} placeholder="Describe your CRM platform, skills needed, and engagement model preference..." value={cForm.project} onChange={e => setC("project", e.target.value)} onBlur={cBlur("project")} />
                    {cTouched.project && <FieldErr msg={cErr.project} />}
                  </div>
                  <button className="btn-teal" type="submit" disabled={cLoading} style={{ width: "100%", padding: isMobile ? "13px" : "15px", borderRadius: 10, border: "none", background: cLoading ? "rgba(0,201,167,0.5)" : `linear-gradient(135deg,${T},${TD})`, color: "#000", fontWeight: 800, fontSize: isMobile ? "14px" : "15px", fontFamily: "'Poppins',sans-serif", cursor: cLoading ? "wait" : "pointer", opacity: cLoading ? 0.7 : 1 }}>
                    {cLoading ? "Sending…" : "Submit — Find My Developer →"}
                  </button>
                  <p style={{ color: "rgba(255,255,255,0.3)", fontSize: isMobile ? "10px" : "11.5px", textAlign: "center", marginTop: 12 }}>🔒 Your information is 100% secure and never shared.</p>
                </form>
              )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 16 : 20 }}>
              <div style={{ background: "rgba(0,201,167,0.05)", border: "1px solid rgba(0,201,167,0.18)", borderRadius: 18, padding: isMobile ? "20px 18px" : "28px 26px" }}>
                <h3 style={{ color: "#fff", fontSize: isMobile ? "15px" : "16px", fontWeight: 800, marginBottom: isMobile ? 14 : 18 }}>What Happens After You Submit?</h3>
                {[{ s: "1", text: "We review your requirements within a few hours." }, { s: "2", text: "We match you with 2-3 pre-vetted developers (24-48hrs)." }, { s: "3", text: "Interview candidates and choose your preferred developer." }, { s: "4", text: "Start with a paid trial or full engagement — your choice." }].map((st, i) => (
                  <div key={i} style={{ display: "flex", gap: isMobile ? 10 : 14, alignItems: "flex-start", marginBottom: i < 3 ? (isMobile ? 12 : 16) : 0, padding: isMobile ? "10px 12px" : "12px 14px", borderRadius: 10, background: "rgba(255,255,255,0.03)", transition: "background .2s", cursor: "default" }}
                    onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = "rgba(0,201,167,0.07)"}
                    onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.03)"}>
