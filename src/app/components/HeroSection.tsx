...
export default function HeroSection() {
  ...
  const getHeroPadding = () => {
    ...
    if (isTablet) return "100px 28px 70px";
    if (isLaptop) return "120px 36px 80px";
    return "120px 48px 80px";
  };

  const getTitleSize = () => {
    if (isMobile) return "clamp(32px, 8vw, 40px)";
    if (isTablet) return "clamp(40px, 5vw, 48px)";
    if (isLaptop) return "clamp(44px, 4.5vw, 52px)";
    ...
    <>
      <style>{`
        ...
        @media (max-width: 960px) {
          .hero-inner    { padding: 50px 28px 48px !important; }
          .hero-grid     { grid-template-columns: 1fr !important; gap: 40px !important; }
          .hero-form-col { display: block !important; max-width: 520px !important; margin: 0 auto !important; }
          .stats-grid    { grid-template-columns: 1fr 1fr !important; }
          .stat-card     { border-right: none !important; border-bottom: 1px solid rgba(255,255,255,.07) !important; padding: 24px 16px !important; }
          .hero-btn-primary, .hero-btn-outline { 
        ...
        @media (max-width: 600px) {
          ...
          .hero-grid     { gap: 32px !important; }
          .hero-form-col { padding: 24px 20px !important; }
          .hero-btns     { 
          ...
          <div className="hero-grid" style={{ 
            ...
            <div>
              ...
              <p className="hero-fade hero-fade-3" style={{
                ...
                fontSize: isMobile ? 15 : isTablet ? 16 : 16.5, 
            ...
            <div className="hero-form-col" style={{
              ...
              padding: isMobile ? "24px 20px" : isTablet ? "30px 24px" : "36px 30px", 
              ...
              {submitted ? (
                <div style={{ textAlign:"center", padding: isMobile ? "30px 0" : "50px 0" }}>
                  <div style={{ fontSize: isMobile ? 48 : 52, marginBottom:16 }}>✅</div>
                  <p style={{ color:TEAL, fontSize: isMobile ? 18 : 19, fontWeight:700, marginBottom:8 }}>We&apos;ll be in touch!</p>
                  <p style={{ color:"rgba(255,255,255,.4)", fontSize: isMobile ? 12 : 13 }}>Expect a reply within 24 business hours.</p>
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap: isMobile ? 12 : 10 }}>
                  ...
                  <textarea className="h-inp"
                    style={{ ...inp, resize:"vertical", minHeight: isMobile ? 80 : 80 }}
                    rows={3} placeholder="Tell us about your project…"
                    value={form.message} onChange={set("message")}
                  />
